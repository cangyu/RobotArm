...
const float max_speed[ROBOTIC_ARM_NUM]={20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0};// degree/s
...
{
  	int delta_pwm=target_pwm[index]-cur_pwm[index];//PWM count
	if(abs(delta_pwm)<pwm_resolution[index])
	  	return;
	
	const float pwm_scale_factor=1.0*delta_pwm/PWM_SPAN;
	
	float delta_t=fabs((target_angle[index]-cur_angle[index]))/max_speed[index]*1000.0;//ms
	const float t_scale_factor=1.0*TIME_SPAN/delta_t;
	
	int ti=0;
	float tf=0.0;
	while(fabs(TIME_SPAN-tf)>1e-6)
	{
	  	tf+=t_scale_factor;
		ti=(int)tf;
	  	
		next_pwm[index]=(int)(sc[ti]*pwm_scale_factor);
...
{
	if(next_pwm[i]>cur_pwm[i])
		cur_pwm[i]=min(target_pwm[i], next_pwm[i]);
	else
		cur_pwm[i]=max(target_pwm[i], next_pwm[i]);
